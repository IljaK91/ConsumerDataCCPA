...
function solve_firm_problem_nl(par::Pars_static; type::Symbol)
...
"""
function comp_statics(τ_set, v_set, A_G_common_set, ξ_set; tol = 1e-10, α_S = 0.8, α_U = 0.6, α_S_L = 1 - α_S, α_U_L = 1 - α_U)
    grid = gridmake(τ_set, v_set, A_G_common_set, ξ_set)
    sol = Array{Any}(undef, 0)
    for i in 1:size(grid)[1]
        @show i
        @show grid[i, :]
        if i == 1
            push!(sol, find_steady_state(Pars_static(τ = grid[i, 1], v = grid[i, 2], A_G_common = grid[i, 3], ξ = grid[i, 4], α_S = α_S, α_U = α_U, α_S_L = α_S_L, α_U_L = α_U_L); tol))
        else
            push!(sol, find_steady_state(Pars_static(τ = grid[i, 1], v = grid[i, 2], A_G_common = grid[i, 3], ξ = grid[i, 4], Y = sol[i-1].Y, w = sol[i-1].w, w_G = sol[i-1].w_G, α_S = α_S, α_U = α_U, α_S_L = α_S_L, α_U_L = α_U_L); tol))
        end
    end
    return sol
end
