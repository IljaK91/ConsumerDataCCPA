...
x = τ_set
xlabel = L"Iceberg Transportation Costs $\tau$"
...
plot_comp_static_firm(x, [prof_LU, prof_BU, prof_LS, prof_BS]; xlabel = xlabel, ylabel = "Profits", Filename = "Profits_tau_GE")
...
plot_comp_static_firm(x, [D_LU, D_BU, D_LS, D_BS]; xlabel = xlabel, ylabel = "Data Bundle", Filename = "DataBundle_tau_GE")
...
plot_comp_static_firm(x, [D_I_LU, D_I_BU, D_I_LS, D_I_BS]; xlabel = xlabel, ylabel = "Internal Data", Filename = "Internal_Data_tau_GE")
...
plot_comp_static_firm(x, [D_E_LU, D_E_BU, D_E_LS, D_E_BS]; xlabel = xlabel, ylabel = "External Data", Filename = "External_Data_tau_GE")
...
plot_comp_static_firm(x, [D_S_LU, D_S_BU, D_S_LS, D_S_BS]; xlabel = xlabel, ylabel = "Shared Data", Filename = "Shared_Data_tau_GE")

p_D_tau = [par.p_D for par in pars]
p_D_tau_2 = [par.p_D/(1-par.τ) for par in pars]
w_G_tau = [par.w_G for par in pars]
w_tau = [par.w for par in pars]
Y_tau = [par.Y for par in pars]

p1 = plot(x, [p_D_tau, p_D_tau_2], ylabel = L"Price of Data $p^D$", label = ["Sell Price" "Buy Price"], color = [:black :red], ls = [:solid :dash], legend = :topleft,right_margin = 1cm)
p3 = plot(x, w_G_tau, ylabel = L"Wage Gathering $w_G$", label = "", color = :black)
p4 = plot(x, w_tau, ylabel=L"Wage $w$", label="", color=:black, right_margin=1cm)
p5 = plot(x, Y_tau, ylabel = L"Aggregate Output $Y$", label = "", color = :black)

plot(p1, p3, p4, p5, size = (800, 600), color = :black, xlabel = xlabel)

savefig("Graphs/Aggregate_GE_tau.pdf")
savefig("Graphs/Aggregate_GE_tau.png")
...
savefig("Graphs/FirmGraphProfits_GE_tau.png")
savefig("Graphs/FirmGraphProfits_GE_tau.pdf")
...
savefig("Graphs/FirmGraphDataNorm_GE_tau.png")
savefig("Graphs/FirmGraphDataNorm_GE_tau.pdf")
...
savefig("Graphs/FirmGraphData_GE_tau.png")
savefig("Graphs/FirmGraphData_GE_tau.pdf")